...
public class AudioChannel
{
    ...
    public AudioChannel(int channel)
    {
        ...
        trackContainer.SetParent(AudioManager1.instance.transform);
    }
    ...
    public AudioTrack PlayTrack(AudioClip clip, bool loop, float startingVolume, float volumeCap, float pitch, string filePath)
    {
        ...
        AudioTrack track = new AudioTrack(clip, loop, startingVolume, volumeCap, pitch, this, AudioManager1.instance.musicMixer);
        track.Play();
    ...
    {
        if (!isLevelingVolume)
            co_volumeLeveling = AudioManager1.instance.StartCoroutine(VolumeLeveling());
    }
    ...
    private IEnumerator VolumeLeveling()
    {
        while ((activeTrack != null && (tracks.Count > 1 || activeTrack.volume != activeTrack.volumeCap)) || (activeTrack == null && tracks.Count > 0))
        {
            for (int i = tracks.Count - 1; i >= 0; i--)
            {
                ...
                track.volume = Mathf.MoveTowards(track.volume, targetVol, AudioManager1.TRACK_TRANSITION_SPEED * Time.deltaTime);
