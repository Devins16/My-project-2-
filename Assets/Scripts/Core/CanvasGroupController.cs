...
public class CanvasGroupController
{
    ...
    private IEnumerator Fading(float aplha, float speed = 1f, bool immediate = false)
    {
        ...
        while (cg.alpha != aplha)
        {
            cg.alpha = Mathf.MoveTowards(cg.alpha, aplha, Time.deltaTime * DEFAULT_FADE_SPEED * speed);
            yield return null;
        }
