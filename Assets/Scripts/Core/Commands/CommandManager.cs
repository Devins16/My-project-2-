...
namespace COMMANDS
{
    ...
    {
        private const char SUB_COMMAND_INDENTIFIER = '.';
        ...
        private CommandProcess topPocess => activeProcesses.Last();
        ...
        {
            if (commandName.Contains(SUB_COMMAND_INDENTIFIER))
        ...
        {
            string[] parts = commandName.Split(SUB_COMMAND_INDENTIFIER);
            string databaseName = string.Join(SUB_COMMAND_INDENTIFIER, parts.Take(parts.Length - 1));
        ...
        {
            CommandProcess process = topPocess;
