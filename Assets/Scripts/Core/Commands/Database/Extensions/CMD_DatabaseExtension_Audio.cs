...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        private static void PlaySFX(string[] data)
        {
            ...
            AudioManager1.instance.PlaySoundEffect(sound, volume: volume, pitch: pitch, loop: loop);
        }

        private static void StopSFX(string data)
        {
            AudioManager1.instance.StopSoundEffect(data);
        }
        ...
        private static void PlayVoice(string[] data)
        {
            ...
            AudioManager1.instance.PlayVoice(sound, volume: volume, pitch: pitch, loop: loop);
        }
        ...
        private static void PlayTrack(string filepath, int channel, CommandParameters parameters)
        {
            ...
            parameters.TryGetValue(PARAM_START_VOLUME, out startVolume, defaultValue: 1f);
            ...
            AudioManager1.instance.PlayTrack(sound, channel, loop,startVolume, volumeCap, pitch, filepath);
        }
        ...
        private static void StopTrack(string data)
        {
            if (int.TryParse(data, out int channel))
                AudioManager1.instance.StopTrack(channel);
            else
                AudioManager1.instance.StopTrack(data);
        }
    }
}